...
from aws_utils import download_file_from_s3, get_param
...
class Trainer:
    ...
    def download_data_from_s3(self) -> None:
        download_file_from_s3(
            ...
            config.s3_train_set_path,
            str(config.train_set_path),
        )
        download_file_from_s3(
            config.s3_bucket_name,
            config.s3_cross_val_set_path,
            str(config.cross_val_set_path),
        )

        download_file_from_s3(
            config.s3_bucket_name,
            config.s3_benchmark_data_dir + "/train_benchmark.parquet",
            str(config.train_set_benchmark_path),
        )
        download_file_from_s3(
            config.s3_bucket_name,
            config.s3_benchmark_data_dir + "/cross_val_benchmark.parquet",
            str(config.cross_val_set_benchmark_path),
        )
    ...
    def hyperparam_tuning(self, train: pd.DataFrame, cross_val: pd.DataFrame) -> Dict[str, float]:
        neptune_api_token = get_param("/yt-trending/secrets/NEPTUNE_API_KEY")
    ...
    def train_final_model(
        ...
        model_registry = neptune.init_model(
            with_id="YTTREN-XGB",
            project=config.neptune_project_name,
            api_token=neptune_api_token,
        )
