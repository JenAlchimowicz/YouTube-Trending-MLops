...
import numpy as np
import pandas as pd
from aws_utils import download_file_from_s3, list_files_in_s3_directory, upload_df_to_s3_parquet
...
class FeatureStoreSupervisor:
    ...
    def download_data_from_s3(self) -> None:
        ...
        for key in list_files_in_s3_directory(config.s3_bucket_name, config.s3_artifacts_dir):
            filename = Path(key).name
            download_file_from_s3(
                config.s3_bucket_name,
                key,
                config.artifacts_dir / filename,
            )
    ...
    def save_feature_store_data_to_s3(self, fs_channels: pd.DataFrame, fs_category: pd.DataFrame) -> None:
        if not config.feature_store_dir.exists():
            config.feature_store_dir.mkdir(parents=True)
